/* $Id: interface.c 2350 2007-01-13 10:12:31Z nick $
 *
...
GtkWidget* create_main_window (void)
{
	...
	gtk_window_set_title (GTK_WINDOW (window), "gtaskmanager");
...
void show_about_dialog(void)
{
	const gchar *authors[] = { \
		...
		"Olivier Duclos <oliwer@free.fr>", NULL };
	
	gtk_show_about_dialog (NULL, 
			"program-name", PACKAGE,
			"version", VERSION,
			"copyright", "Copyright (c) 2007 Olivier Duclos",
			"authors", authors,
			"comments", "A lightweight process manager.",
			NULL);
}
...
void send_signal_to_task(gchar *task_id, gchar *signal)
{
	if(task_id != "" && signal != NULL)
	{
		...
		if(system(command) != 0) {
			GtkWidget *dialog_err = gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_ERROR, 
			                        GTK_BUTTONS_OK, "Couldn't %s the task with ID %s", signal, task_id);
			gtk_dialog_run(dialog_err);
